...
import pytest
import src.news.news_source.news_source_extract as news_source
...
def mock_requests_get():
    with patch('src.news.news_source.news_source_extract.requests.get') as mock_get:
        yield mock_get
...
def test_extract_success(mock_requests_get):
    ...
    result, row_count = news_source.extract('app_key', 'headers', 'endpoint', 'params')
...
def test_extract_failure(mock_requests_get):
    ...
    with pytest.raises(Exception, match=r'Response status: 400'):
        news_source.extract('app_key', 'headers', 'endpoint', 'params')


@pytest.fixture
def mock_azure_blob_handler():
    with patch('src.news.news_source.news_source_extract.AzureBlobHandler') as mock_handler:
        yield mock_handler
...
def test_load(mock_azure_blob_handler):
    ...
    result_batch_no = news_source.load(container_details, 'file_content')
