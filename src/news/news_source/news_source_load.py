...
def load_files(file_name, file_content):
    ...
    for row in file_content:
        cursor.execute('''INSERT INTO news.news_source_src (source_id, name, url, description, language, country, source_file_name, batch_no) VALUES (?, ?, ?, ?, ?, ?, ?, ?);''', row['id'], row['name'], row['url'], row['description'], str(row['language']), str(row['country']), file_name, batch_no)
        cnx.commit()
